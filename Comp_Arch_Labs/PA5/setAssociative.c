...
void accessData (
    ...
    mem_addr_t set = (addr >> b) & 0x03; //This give 00000011 in binary

    // Find the correct cache set
    cache_set_t currSet = cache[set];

    // Cache hit
    for(unsigned int i = 0; i < E; i++){
        if(currSet[i].tag == tag && currSet[i].valid){
            (*hit_count)++;
            currSet[i].recent_use = program_counter;
            return;
        }
    }
    ...
    for(unsigned int i = 1; i < E; i++){
        if(currSet[i].recent_use < currSet[placement].recent_use){
            placement = i;
        }
    }
    ...
    if(currSet[placement].valid){
        (*eviction_count)++;
    }
    else{
        currSet[placement].valid = true;
    }
    currSet[placement].tag = tag;
    currSet[placement].recent_use = program_counter;
}
